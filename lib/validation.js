...
export function datetimeInput(value, { label, required = false }) {
  ...
  return `${datePart}T${dateTimeMatch[2]}:${dateTimeMatch[3]}`;
}
...
export function tableCountInput(value, { label = "La cantidad de mesas", max = 60 } = {}) {
  const text = String(value ?? "").trim();
...
export function tableNumberInput(value, { label = "La mesa", max }) {
  const text = String(value ?? "").trim();
  ...
  if (Number.isInteger(max) && number > max) {
    throw new Error(`${label} debe ser como maximo ${max}.`);
  }
...
export function moneyInput(
  value,
  { label = "El monto", required = false, min = 0, max = 10000000000 } = {},
) {
  const text = String(value ?? "").trim();
  ...
  if (!/^\d+$/.test(text)) {
    throw new Error(`${label} debe ser un numero entero en pesos.`);
  }

  const number = Number(text);

  if (number < min || number > max) {
    throw new Error(`${label} debe ser un numero entre ${min} y ${max}.`);
  }

  return number;
}

export function shortNameInput(
  value,
  { label, min = 2, max = 60 } = {},
) {
  const text = String(value ?? "").trim();
