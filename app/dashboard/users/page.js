      ...
      <section className="mx-auto mt-7 grid w-full max-w-6xl gap-6 px-4 sm:px-6 lg:px-8 xl:grid-cols-[minmax(280px,360px)_1fr]">
        ...
        <div>
          <div className="mb-3 flex items-center justify-between gap-3">
            ...
            <span className="text-sm text-brand">{users.length} total</span>
          </div>
          ...
          {users.length === 0 ? (
            ...
            <div className="grid gap-px overflow-hidden border border-accent bg-accent">
              {users.map((managedUser) => (
                <article
                  className="grid min-w-0 gap-4 bg-surface p-4 sm:p-5 lg:grid-cols-[minmax(0,1fr)_auto]"
                  key={managedUser.uid}
                >
                  <div className="min-w-0">
                    ...
                    <p className="mt-3 overflow-wrap-anywhere font-mono text-xs text-brand/60">
                      {managedUser.uid}
                    </p>
                  ...
                  <div className="grid gap-2 sm:flex sm:flex-wrap sm:items-start lg:justify-end">
                    <Link
                      className="inline-flex h-9 w-full items-center justify-center border border-accent px-3 text-sm font-semibold text-ink transition hover:border-secondary hover:bg-secondary/10 sm:w-auto"
                    ...
                    {managedUser.uid !== user.uid ? (
                      <form action={deleteUser.bind(null, managedUser.uid)}>
                        <button
                          className="h-9 w-full border border-brand/40 px-3 text-sm font-semibold text-brand transition hover:bg-brand/10 sm:w-auto"
                          type="submit"
                        >
                          Eliminar
                        </button>
