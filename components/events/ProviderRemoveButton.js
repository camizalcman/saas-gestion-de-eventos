...
export default function ProviderRemoveButton({
  ...
  function handleSubmit(event) {
    ...
    startTransition(async () => {
      try {
        ...
        setError(
          submitError?.message || "No se pudo quitar el proveedor.",
        );
      }
    });
  }
  ...
  return (
    <>
      ...
      {open ? (
        <div
          aria-labelledby={`remove-provider-title-${providerName}`}
          aria-modal="true"
        ...
        >
          ...
          >
            <h2
              ...
              id={`remove-provider-title-${providerName}`}
            >
              ¿Quitar proveedor?
            </h2>
            <p className="mt-2 text-sm leading-6 text-brand">
              ¿Estás segura de eliminar a{" "}
              <span className="font-semibold text-ink">{providerName}</span>?
