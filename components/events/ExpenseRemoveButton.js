...
export default function ExpenseRemoveButton({
  ...
  function handleSubmit(event) {
    ...
    startTransition(async () => {
      try {
        ...
        setError(
          submitError?.message || "No se pudo eliminar el gasto.",
        );
      }
    });
  }
  ...
  return (
    <>
      ...
      {open ? (
        <div
          aria-labelledby={`remove-expense-title-${expenseTitle}`}
          aria-modal="true"
        ...
        >
          ...
          >
            <h2
              ...
              id={`remove-expense-title-${expenseTitle}`}
            >
              ¿Eliminar gasto?
            </h2>
            <p className="mt-2 text-sm leading-6 text-brand">
              ¿Estás segura de eliminar el gasto{" "}
              <span className="font-semibold text-ink">{expenseTitle}</span>?
