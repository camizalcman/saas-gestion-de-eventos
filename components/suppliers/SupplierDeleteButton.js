...
export default function SupplierDeleteButton({ action, supplierName }) {
  ...
  function handleSubmit(event) {
    ...
    startTransition(async () => {
      try {
        ...
        setError(
          submitError?.message || "No se pudo eliminar el proveedor.",
        );
      }
    });
  }
  ...
  return (
    <>
      ...
      {open ? (
        <div
          aria-labelledby={`delete-supplier-title-${supplierName}`}
          aria-modal="true"
        ...
        >
          ...
          >
            <h2
              ...
              id={`delete-supplier-title-${supplierName}`}
            >
              Eliminar proveedor
            </h2>
            <p className="mt-2 text-sm leading-6 text-brand">
              ...
              de la galería? Esta acción no se puede deshacer.
            </p>
            {error ? (
